...
| **UnSafePerfectHashTable** | Two-level perfect hash table (FKS). Built from a **static** set of keys; strict **O(1)** lookups. |
| **VanEmdeBoas** | van Emde Boas tree. Two modes: **dynamic** (Cuckoo-based, inserts on the fly) and **static** (UnSafePerfectHashTable-based, built from a known key set). |
...
unsafe
{
    ...
    UnSafePerfectHashTable* table = UnSafePerfectHashTable.Create(keys, values);

    // Strict O(1) lookup
    UnSafePerfectHashTable.Entry* entry = UnSafePerfectHashTable.Find(table, key: 42);
    if (entry != null)
...
Static mode builds the tree in a single pass from a known key set. Clusters are indexed using `UnSafePerfectHashTable` (FKS) instead of `CuckooHashTable`, which enables faster and more compact access when the set is fixed.
...
Unit tests live in `tests/MATTAR.PerformanceCollections.Tests` and cover `CuckooHashTable`, `UnSafePerfectHashTable`, `VanEmdeBoas` (dynamic and static modes), and the low-level `UnSafeVanEmdeBoas` API.
...
Comparative benchmarks are available in `benchmarks/` and measure `CuckooHashTable` and `UnSafePerfectHashTable` against standard .NET collections (`Dictionary`, `HashSet`, etc.).
